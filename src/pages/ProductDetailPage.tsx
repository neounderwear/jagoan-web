...
const ProductDetailPage: React.FC = () => {
  ...
  if (!product) {
    return (
      <div className="text-center py-20">
        ...
        <Link to="/katalog" className="mt-4 inline-block text-primary hover:underline">
          Kembali ke Katalog
        </Link>
      </div>
    );
  ...
  const waNumber = "6281317056067";
  let waMessage = `Halo Jagoan Underwear, saya tertarik dengan produk ${product.name}.`;
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-x-12 gap-y-10">
            ...
            <div className="mt-4 grid grid-cols-5 gap-4">
              {product.images.map((image, index) => (
                <button
                  ...
                  className={`aspect-w-1 aspect-h-1 w-full rounded-md overflow-hidden bg-gray-100 focus:outline-none focus:ring-2 focus:ring-offset-2 transition-all ${selectedImage === image ? "ring-primary" : "ring-transparent hover:opacity-80"}`}
                >
          ...
          <div className="flex flex-col">
            ...
            <div className="mt-4 flex items-baseline space-x-3">
              <p className="text-3xl tracking-tight text-red-600 font-bold">{formatCurrency(price)}</p>
              {originalPrice && <p className="text-xl text-gray-500 line-through">{formatCurrency(originalPrice)}</p>}
            </div>

            <div className="mt-8 border-t border-gray-200 pt-8">
              <h3 className="text-sm font-medium text-gray-900">{product.variants[0]?.type || "Varian"}</h3>
                ...
                {product.variants[0]?.values.map((variant) => (
                  <button
                    ...
                    className={`px-4 py-2 border rounded-md text-sm font-medium transition-colors
                      ${selectedVariant === variant.value ? "bg-primary text-white border-primary" : "bg-white text-gray-900 border-gray-200"}
            ...
            <a
              ...
              className={`mt-10 w-full rounded-md px-8 py-3 text-base font-medium text-white shadow-sm transition-colors text-center
                ${!selectedVariant ? "bg-gray-400 cursor-not-allowed" : "bg-green-600 hover:bg-green-700"}`}
            >
