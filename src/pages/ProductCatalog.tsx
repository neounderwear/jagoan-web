    ...
    <div className="bg-gray-50 min-h-screen font-poppins">
      <header className="sticky top-0 z-30 bg-white/90 backdrop-blur-sm border-b border-gray-200">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-4 flex items-center justify-between">
          ...
          <button onClick={() => setIsFilterOpen(true)} className="lg:hidden rounded-lg bg-primary px-4 py-2 text-sm font-medium text-white hover:bg-primary/90 flex items-center shadow-sm">
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col lg:flex-row gap-8 py-8">
          <aside className={`fixed top-0 left-0 h-full w-80 bg-white shadow-xl z-50 p-6 transform transition-transform duration-300 ease-in-out lg:hidden ${isFilterOpen ? "translate-x-0" : "-translate-x-full"}`}>
            <div className="flex justify-between items-center mb-8">
              <h2 className="text-lg font-bold">Filter & Urutkan</h2>
              <button onClick={() => setIsFilterOpen(false)} className="p-2 -mr-2 rounded-full hover:bg-gray-100">
          ...
          <main className="w-full lg:w-3/4 xl:w-4/5">
            {loading ? (
              ...
              <div className="text-center py-20 col-span-full bg-white rounded-xl border border-gray-200">
                <p className="font-semibold text-gray-800">Produk Tidak Ditemukan</p>
                <p className="text-sm text-gray-500 mt-2">Coba gunakan kata kunci lain atau hapus filter.</p>
              </div>
            )}
          </main>
        </div>
